...
if [[ "${DOCKER_BUILD}" == '1' ]]; then
    ...
    sudo $(which docker-compose) pull caddy-service &>> ${CWD}/netflix-proxy.log
    log_action_end_msg $?
else
...
sudo $(which sqlite3) ${CWD}/auth/db/auth.db 'INSERT INTO USERS (privilege, expires, username, password) VALUES (1, "2039-12-31", "user", "$pbkdf2-sha256$200000$yxmjFMKY05pTinEupRQC4A$1ZP2BKR.BRnEdBw7G1Nku.BXCFcY3NpEqFn3srJZSXY");' &>> ${CWD}/netflix-proxy.log

printf "\nnetflix-proxy-admin site=http://${EXTIP6}:8080/ API credentials=\e[1muser:smile\033[0m\n"
